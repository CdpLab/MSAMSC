...
from ICA import I_acoustic,I_visual

wandb.login(key="2e5666bb36f785e3e06b7dca813fefb9baa1ec47")
WANDB_MODE="offline"
def parser_args():
...
def prep_for_training(args, num_train_optimization_steps: int):
    config = RobertaConfig.from_pretrained(args.model_name_or_path, num_labels=1, finetuning_task='sst') #情感分析任务微调
    ...
    CE_params = ['CE']
    optimizer_grouped_parameters = [ #选择参数
        {
            "params": [
...
def train_epoch(args, model: nn.Module, train_dataloader: DataLoader, optimizer, scheduler):
    ...
    nb_tr_steps = 0
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):#input_size([64,50])
        ...
        textagument_model = CrossSelfTransformer(latent_dim = 35,input_dim = 74,heads = 8 ,dim_head=8)
        #audio_model = CrossSelfAduioTransformer(latent_dim = 74,input_dim = 35,heads = 8 ,dim_head=8)
        #a_model = audio_model.to(DEVICE)
        t_model = textagument_model.to(DEVICE)
        #a_ = a_model(acoustic,visual)
        v_ = t_model(visual,acoustic)
        # 插入一个门控单元判断相似性

        #comparator1 = ModalSimilarityComparator(threshold=0.9)
        comparator = ModalSimilarityComparator(threshold=0.7)
        #if comparator1.compare(a_,acoustic):
             #acoustic = a_
        if comparator.compare(v_,visual):
             visual = v_
        #input_ids_1 = t_model(input_ids,visual,acoustic)
        #input_ids_2 = input_ids
        #input_ids_3 = input_ids_1+input_ids_2
        #input_ids = torch.mean(input_ids_3)
        #input_ids = input_ids_1.to(torch.int)
        #acoustic = I_acoustic(acoustic.cpu())
        #visual   = I_visual(visual.cpu()) 
        outputs = model(
            input_ids,
            visual,
            acoustic,
        ...
        logits = outputs[0]
        loss_fct = MSELoss()   #MASE损失
        loss = loss_fct(logits.view(-1), label_ids.view(-1))
        if args.gradient_accumulation_step > 1:
            loss = loss / args.gradient_accumulation_step
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)    #clip裁剪
        tr_loss += loss.item()
        nb_tr_steps += 1
...
def main():
    ...
    model, optimizer, scheduler = prep_for_training(args, num_train_optimization_steps)  #设置模型、参数优化器、调度器
