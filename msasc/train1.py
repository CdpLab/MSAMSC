...
form emotion_constraint_loss import EmotionConstraintLoss
...
class CenterLoss(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        # Compute center loss
        center_loss = torch.sum((features - self.centers[labels]) ** 2) / batch_size
        return center_loss
...
def train_epoch(args, model: nn.Module, train_dataloader: DataLoader, optimizer, scheduler):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):#input_size([64,50])
        ...
        logits = outputs[0]
        loss_fct = MSELoss()   
        center_loss_1 = center_loss(logits.view(-1), label_ids.view(-1))
        loss = loss_fct(logits.view(-1), label_ids.view(-1)) + 0.1*center_loss_1 
